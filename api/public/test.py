...
connection_string = "mongodb+srv://Admin:CISY9zab45Vd71YT@cluster0.wwhbk9o.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"

def upload_csv():
    try:
        # Vercel-friendly path to CSV files
        csv_dir = os.path.join(os.path.dirname(__file__), 'public')

        LTM1100 = pd.read_csv(os.path.join(csv_dir, 'LTM1100_4_2.csv')).fillna(0)
        LTM1500 = pd.read_csv(os.path.join(csv_dir, 'LTM1500_8_1.csv')).fillna(0)

        LTM1100 = LTM1100[LTM1100.ne(0).any(axis=1)]
        LTM1500 = LTM1500[LTM1500.ne(0).any(axis=1)]

        working_radii_dict = {"LTM1100": list(LTM1100.pop(LTM1100.columns[0]))}
        working_radii_dict["LTM1500"] = list(LTM1500.pop(LTM1500.columns[0]))
        LTM1100 = LTM1100.iloc[:, 1:]
        LTM1500 = LTM1500.iloc[:, 2:]
        boom_length_dict = {"LTM1100": LTM1100.columns}
        boom_length_dict["LTM1500"] = LTM1500.columns
        boom_length_dict["LTM1100"] = [float(col.replace('m', '')) for col in boom_length_dict["LTM1100"]]
        boom_length_dict["LTM1500"] = [float(col.replace('m', '')) for col in boom_length_dict["LTM1500"]]
        model_dict = {"LTM1100": LTM1100, "LTM1500": LTM1500}
...
def crane_calculator():
    ...
    response = {'image': plot_image, 'horizontal_arm_angle': math.degrees(horizontal_arm_angle),
                ...
                "check": check, "estimated_lifting_capacity": estimated_lifting_capacity}
    return jsonify(response)
...
def finding_lifting_capacity(model, indexes, working_radius, boom_length, nearest_lifting_capacity):
    ...
    # print(lower_working_radius_lifting_capacity, higher_working_radius_lifting_capacity)
    return (ans)
